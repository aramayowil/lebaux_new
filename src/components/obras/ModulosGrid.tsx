...
import { Interior, Marco, Producto, Vidrio } from "@/types";
...
const TIPO_COLORS: Record<TipoModulo, string> = {
  ...
  vacio:
    "bg-steel-50 border-steel-200 dark:bg-steel-800/40 dark:border-steel-700",
};

const TIPO_CHIP: Record<
  TipoModulo,
  "primary" | "warning" | "secondary" | "default"
> = {
  vidrio: "primary",
  panel: "warning",
  persiana: "secondary",
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-semibold text-steel-500 uppercase tracking-wide">
        Módulos — {nRows} fila{nRows !== 1 ? "s" : ""} × {nCols} col
        {nCols !== 1 ? "umnas" : "umna"}
      </p>
        ...
        <div className="flex gap-1 pl-7 pr-14">
          {cols.map((w, col) => (
            <div
              ...
              style={{ flex: Math.max(1, Math.round(w / 100)) }}
            >
              <span className="text-[10px] text-steel-400 font-mono">
                {Math.round(w)} mm
              </span>
...
function ModuloCellEditor({
  ...
  const marcosDeProducto = marcos.filter(
    (m: Marco) => m.id_producto === modulo.id_producto,
  );
    ...
    <div className="px-2 pb-3 pt-1 space-y-2 border-t border-black/10 dark:border-white/10">
      ...
      <Select
        ...
        onSelectionChange={(k: Set<string>) =>
          onUpdate({ tipo: [...k][0] as TipoModulo })
        }
        classNames={{
          trigger:
            "bg-white dark:bg-steel-900 border border-steel-200 dark:border-steel-700 h-8 min-h-unit-8 text-xs",
        }}
      >
      ...
      {modulo.tipo === "vidrio" && (
        <Select
          ...
          size="sm"
          selectedKeys={modulo.id_vidrio ? [modulo.id_vidrio] : []}
          onSelectionChange={(k: Set<string>) =>
            onUpdate({ id_vidrio: ([...k][0] as string) || null })
          }
          classNames={{
            trigger:
              "bg-white dark:bg-steel-900 border border-steel-200 dark:border-steel-700 h-8 min-h-unit-8 text-xs",
      ...
      {(modulo.tipo === "panel" || modulo.tipo === "persiana") && (
        <>
          <Select
            ...
            size="sm"
            selectedKeys={
              modulo.id_producto ? [String(modulo.id_producto)] : []
            }
            onSelectionChange={(k: Set<string>) => {
              const id = parseInt([...k][0] as string) || null;
              onUpdate({ id_producto: id, id_marco: null, id_interior: null });
            }}
            classNames={{
              trigger:
                "bg-white dark:bg-steel-900 border border-steel-200 dark:border-steel-700 h-8 min-h-unit-8 text-xs",
          ...
          {modulo.id_producto && (
            <Select
              ...
              placeholder="Sin asignar"
              size="sm"
              selectedKeys={modulo.id_marco ? [String(modulo.id_marco)] : []}
              onSelectionChange={(k: Set<string>) =>
                onUpdate({ id_marco: parseInt([...k][0] as string) || null })
              }
              classNames={{
                trigger:
                  "bg-white dark:bg-steel-900 border border-steel-200 dark:border-steel-700 h-8 min-h-unit-8 text-xs",
